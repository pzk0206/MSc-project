...
- 跨两条或以上实验流程复用的稳定逻辑放入 `src/shared/`，避免复制实现。
...
- 传统计算机视觉、VLM + 几何后端和 VLM + CNN 后端三种方法。
- 统一评估标准。
- 项目贡献、关键发现和范围边界。
...
由现有 `docs/failure_analysis.md` 移动并统一命名而来，保留 VLM 引导的几何实验流程的失败模式、统计和结论。
...
如果不同文档中的数字不一致，以当前 `README.md` 中已更新的汇总结果为最新状态，并在 `CURRENT_STATUS.md` 标注结果口径，例如单次实验或五次实验均值 ± 标准差。
...
- Codex 在新运行或新会话中自动读取根目录 `AGENTS.md`，并按其中约定加载相应的 AI 上下文文档。
