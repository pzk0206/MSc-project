"""Execute stage 3: descend open Panda fingers toward the truth cube."""
...
class TruthApproachConfig:
    """Deterministic stage-3 scene, target, and stability settings."""
    ...
    def __post_init__(self) -> None:
        if self.target_name != "cube":
            raise ValueError("stage 3 target_name must be cube")
    ...
    def to_execution_config(self) -> TruthExecutionConfig:
        """Convert the public stage-3 config to the shared core config."""
...
def run_truth_approach(
    config: TruthApproachConfig,
) -> dict[str, object]:
    """Execute pregrasp then descend open fingers to surface standoff."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Execute stage-3 Panda open truth-cube approach."
    )
