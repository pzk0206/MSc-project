...
# ---------------------------------------------------------------------------
# Side-grasp standoff shim (negative surface-standoff support)
# ---------------------------------------------------------------------------
# The shared ``generate_top_down_pose_candidates`` enforces
# ``surface_standoff_m > 0`` by design (it only emits top-down hover poses).
# This experiment needs a NEGATIVE standoff so the gripper descends below the
# measured top surface and grasps the cube's side near table level:
# ``pose_generation._candidate`` computes ``standoff.z = surface.z + standoff_m``
# (approach axis = -Z), so -0.020 m on the top surface (~0.675 m) yields
# standoff.z ~ 0.655 m, 5 mm above the table (~0.650 m). The pose arithmetic is
# valid for negative standoff; only the positivity guard blocks it.
#
# Workaround (confined to this new runner; shared modules untouched): for a
# negative standoff, delegate to the original generator with ``abs(standoff)``
# (positive, passes the guard) and then shift the whole pose column downward by
# ``2 * standoff`` (the sign-corrected gap to the intended level). This produces
# exactly the poses a direct ``-0.020`` would, without duplicating the generator.
#
# ``execution_plan.PlannedPoseCandidate`` additionally requires a fixed vertical
# ladder: approach.z == grasp_depth.z + 0.015 and pregrasp.z == approach.z + 0.10.
# Lowering the grasp depth to surface.z - 0.020 therefore also lowers the
# approach column (standoff 0.02 above the surface) by
# ``(surface.z - 0.020 + 0.015) - (surface.z + 0.02) = -0.025`` so the ladder
# stays coherent. Positive-standoff calls (the approach candidates) receive this
# reconciliation shift; the depth column receives the 2*standoff shift.
_SIDE_GRASP_DEPTH_STANDOFF_M = -0.020
_APPROACH_RECONCILE_SHIFT_M = (
    _SIDE_GRASP_DEPTH_STANDOFF_M + 0.015 - 0.02
)  # = -0.025
_ORIG_TOP_DOWN_GENERATE = generate_top_down_pose_candidates


def _shift_pose_columns(
    ...
    return tuple(
        replace(
            candidate,
            surface_standoff_pose=replace(
                candidate.surface_standoff_pose,
                position=tuple(
                    value + delta
                    for value, delta in zip(
                        candidate.surface_standoff_pose.position, shift
                    )
                ),
            ),
            pregrasp_pose=replace(
                candidate.pregrasp_pose,
                position=tuple(
                    value + delta
                    for value, delta in zip(candidate.pregrasp_pose.position, shift)
                ),
            ),
        )
...
def generate_top_down_pose_candidates(*args, **kwargs):
    standoff = float(kwargs.get("surface_standoff_m", 0.02))
    if standoff >= 0.0:
        # Approach column: lower it to keep the ladder above the side grasp.
        candidates = _ORIG_TOP_DOWN_GENERATE(*args, **kwargs)
        return _shift_pose_columns(
            candidates, (0.0, 0.0, _APPROACH_RECONCILE_SHIFT_M)
        )
    positive_kwargs = dict(kwargs)
    positive_kwargs["surface_standoff_m"] = abs(standoff)
    candidates = _ORIG_TOP_DOWN_GENERATE(*args, **positive_kwargs)
    return _shift_pose_columns(
        candidates, (0.0, 0.0, 2.0 * standoff)
    )
