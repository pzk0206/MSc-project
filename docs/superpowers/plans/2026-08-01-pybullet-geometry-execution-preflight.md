...
- Stage 6A 按计划完成并通过原定静态安全门控；该门控没有定义预测点相对物体
  质心的误差阈值，因此不应把通过结果扩大解释为物理抓取中心有效。
- 使用 metadata 中已保存、但未参与预测的 cube 真值位姿进行后验计算：预测
  表面点相对 cube 质心的 X/Y 偏差为 `0.026456 m/0.002225 m`，XY 合成偏差
  为 `0.026550 m`；相对 cube 顶面的 Z 偏差为 `0.000509 m`。
- 该现象来自斜视相机下“可见表面命中点”和“物体三维中心”的语义差异，不是
  重投影、segmentation 或射线门控失败。原始产物与计划保持不变，不能使用
  真值静默修正后继续沿用同一个 Stage 6A 结论。
- 后续先增加独立、无电机命令的 Stage 6A.1 后验中心偏差证据，再决定原样执行
  当前计划，或设计一套对 geometry 与多头 CNN 公平共享的中心恢复规则。
