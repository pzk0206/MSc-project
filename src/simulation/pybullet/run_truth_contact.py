"""Execute stage 4: descend, close, and hold truth-cube contact."""
...
class TruthContactConfig:
    """Deterministic stage-4 scene, target, and stability settings."""
    ...
    def __post_init__(self) -> None:
        if self.target_name != "cube":
            raise ValueError("stage 4 target_name must be cube")
    ...
    def to_execution_config(self) -> TruthExecutionConfig:
        """Convert the public stage-4 config to the shared core config."""
...
def run_truth_contact(config: TruthContactConfig) -> dict[str, object]:
    """Descend open fingers, close, and hold bilateral cube contact."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Execute stage-4 Panda bilateral truth-cube contact."
    )
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--gui", action="store_true")
    args = parser.parse_args()
    summary = run_truth_contact(
        TruthContactConfig(
