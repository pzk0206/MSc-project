...
\section{研究范围与意义}
三个主要研究问题仍限定于 Cornell 数据集上的离线二维 RGB 感知，不在这些性能比较中融合深度。论文另以固定 PyBullet 场景进行补充仿真验证，分为三层：(1) 检验九个二维抓取中心能否通过米制深度和相机矩阵恢复为目标表面世界点，即关闭从感知输出到三维坐标的数据流缺口；(2) 以真值姿态验证分阶段运动-接近-闭合-抬升控制链通过后，加载由感知生成的冻结执行计划，完成从 pregrasp 到 lift-hold 的完整物理抓取；(3) 通过斜视与头顶两种相机配置的对照实验，诊断三维中心偏差的根因并验证修正方案的有效性——头顶相机下 XY 偏差从 $26.55$\,mm 降至 $0.76$\,mm，同一感知管线从物理抓取失败翻转为完整成功。该验证不进入 Cornell 排名，且限于单场景 $N=1$、单物体和离线 rectangle metric，不等同于真实机器人抓取成功率。由于缺少权威图像到物体实例映射，本文不把当前 image-wise 五折解释为 object-wise 未见物体泛化。通用提示被固定为控制变量，prompt 搜索与 VLM 微调属于后续工作。

学术上，本研究通过共享定位裁剪减少模块混杂，补充 VLM 前端与轻量抓取后端之间的受控证据。工程上，定位器和后端可以独立替换，便于在算力有限的平台上诊断误差来源。更重要的是，论文同时报告均值、波动和逐样本互补性，使结论强度与现有证据相匹配。

\section{论文结构}
第二章按技术主题批判性回顾抓取矩形、深度学习、开放词汇定位和语言驱动抓取，并提出研究缺口。第三章说明数据、三条流程、训练协议、评价方法、研究伦理及独立的 PyBullet 坐标验证与分阶段物理执行方法。第四章按三个研究问题报告 Cornell 定量和定性结果，并另列补充仿真验证（坐标门控、真值抬升与感知驱动抓取）；第五章联系既有文献解释发现及限制，第六章逐条回答研究问题并提出未来工作。附录记录复现命令、环境、结果产物与固定划分的历史用途。
...
\subsection{头顶相机预检与抓取深度修正（Stage Overhead）}
针对斜视 Stage 6B 揭示的 XY 中心偏差根因，增设头顶（overhead）相机 preflight。相机置于场景正上方（$\textit{eye}=(0.5,0.0,1.3)$，$\textit{target}=(0.5,0.0,0.62)$），光轴垂直向下；采用 $\textit{up}=(0,1,0)$ 以避免标准 $\textit{up}=(0,0,1)$ 与视线平行导致的 view matrix 退化。其余感知管线与 Stage 6A 相同：Grounding DINO 定位（提示 \texttt{red cube}）、geometry 二维抓取、单像素深度反投影，不启用中心恢复（$\textit{center\_recovery}=\textit{None}$）。

头顶方案的核心调整位于抓取深度。原斜视 Stage 6A 将 grasp\_depth 的 TCP 置于 cube 表面上方 $0.005$\,m，夹爪虽建立双指接触但仅触及顶皮，抬升时滑脱。头顶方案将抓取深度 standoff 从 $+0.005$\,m 改为 $-0.025$\,m，使 TCP 下探至 cube 中心高度（$Z=0.650$\,m）；pregrasp offset 同步从 $0.115$\,m 调整为 $0.145$\,m，以保持 pregrasp 到 grasp\_depth 的安全垂直距离。此调整需要三处工程修改：(1) 放开 $\textit{surface\_standoff\_m}>0$ 的原有约束，允许负值表示 TCP 低于目标表面；(2) 将 approach 与 grasp\_depth 的高度差限制从硬编码 $0.015$\,m 放宽为 $\textit{approach}>\textit{grasp\_depth}$；(3) 修改 preflight runner 中调用 $\textit{generate\_top\_down\_pose\_candidates}$ 时分别传入 contact 和 grasp\_depth 的差异化参数。首次尝试 $-0.06$\,m（TCP 降至桌面高度 $Z=0.615$\,m）因穿透桌子导致碰撞审计失败；第二次 $-0.025$\,m preflight 通过：$2/2$ 候选 $\textit{gate\_passed}$，$\textit{minimum\_clearance\_m}=0.002$\,m。

头顶计划随后在 Stage 6B 中执行完整物理抓取链条，所有运动控制参数、门控门槛和审计流程与斜视 Stage 6B 相同。该配置与斜视 Stage 6B 的唯一差异是相机位姿（连带抓取深度修正以适配正确的世界点），其余条件（场景、物体、感知后端和执行协议）保持不变，构成控制变量对照。
...
表~\ref{tab:truth-control-chain} 汇总五个分阶段验证结果。阶段 1 完成中立位→安全点→中立位的空中往返（506 步），末端位置误差 $0.0005$\,m。阶段 2 使用真值 AABB 顶面到达 pregrasp，末端-方块 XY 偏差 $0.0007$\,m。阶段 3 张手下降至接触前高度，实际工具高度与计划 $0.02$\,m 的误差为 $0.0008$\,m。阶段 4 增加短下探后闭合双指，双指在闭合第 93 步首次同时接触，运行结束连续保持 121 步；左右最大法向力分别为 $20.49$\,N 和 $19.15$\,N。阶段 5 冻结夹爪命令后抬升，方块最终上升 $0.1200$\,m（门槛 $0.10$\,m），桌面接触为零，末端-方块最大相对漂移 $0.0014$\,m（门槛 $0.01$\,m），末段双指连续接触 482 步，全部禁止碰撞为零。五阶段全部门控通过，阶段 5 计为一次真值姿态仿真抓取成功。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccc}
    ...
    5 抬升保持 & 1457 & 方块上升/漂移 & 0.12\,m / $1.4\times10^{-3}$\,m & $\geq0.10$\,m / $\leq0.01$\,m \\
    \bottomrule
  \end{tabular}
\end{table}

\subsection{感知预检与中心偏差诊断}
Geometry 的 Grounding DINO 定位框为 $[297,189,344,245]$、分数 $0.8170$、cube IoU $0.8717$。二维抓取中心 $(320.5,217.0)$、角度 $0^\circ$，单像素深度 $0.6838$\,m，反投影世界点为 $(0.5065,0.0022,0.6755)$\,m。两个对称候选均通过 82 状态两阶段碰撞审计，确定性选择 $0^\circ$；静态科学门控通过。然而，使用保存的 cube 真值质心 $(0.48,0.0,0.65)$\,m 和冻结半高 $0.025$\,m 进行后验审计后发现：预测世界点相对质心的 X/Y 偏差为 $0.0265/0.0022$\,m，XY 合成偏差 $0.0266$\,m（$26.55$\,mm），不满足 $0.005$\,m 参考门槛；Z 偏差仅为 $0.0005$\,m。独立 CUDA 重跑确认 RGB 哈希、定位框和偏差值一致。
...
\begin{table}[htbp]
  \centering
  \caption{真值抬升与两个感知后端物理抓取结果对比。感知后端均通过静态和到达门控，但抬升阶段因 XY 中心偏差失败。}
  ...
  \begin{tabular}{lccc}
    ...
    末端-cube 漂移 & $0.0014$\,m & $0.120$\,m & $0.120$\,m \\
    抬升桌面接触 & 0 & 481 & 481 \\
    科学门控 & 通过 & 失败（抬升） & 失败（抬升） \\
    \bottomrule
  \end{tabular}
...
两个后端的一致失败模式确认了根因不在后端选择：斜视相机使单像素反投影的可见表面点偏移 cube 质心超过一个半宽（$25$\,mm），夹爪中心偏离导致仅能触及边缘，无法形成稳定抓取。Z 方向高度恢复准确（相对名义顶面误差 $\leq2.3$\,mm），因此问题完全在 XY 平面。

\subsection{头顶相机物理抓取}
头顶相机的 XY 中心偏差为 $0.76$\,mm（斜视 $26.55$\,mm，改善 $97.1\%$），远低于 $5$\,mm 参考门槛，验证了光轴垂直向下可从根本上消除斜视反投影的平面偏移。表~\ref{tab:overhead-grasp} 将头顶成功与斜视失败并置对照：唯一改变的变量是相机位姿（连带抓取深度修正以适配正确的世界点），其余条件——场景、物体、geometry 后端、运动控制参数和门控门槛——保持不变。

\begin{table}[htbp]
  \centering
  \caption{斜视相机与头顶相机物理抓取对照。同一场景、同一物体、同一后端，仅相机位姿不同。}
...
头顶相机下，pregrasp、approach 和 grasp\_depth 到达位置误差均 $\leq1.0$\,mm，全部通过动态门控。双指首次接触在闭合第 41 步（斜视为 226 步），夹爪扎实包裹 cube 中心而非仅触及边缘。抬升阶段 cube 上升 $0.1199$\,m（门槛 $0.10$\,m），离桌干净（桌面接触仅 2 次），末端-cube 相对漂移仅 $0.0014$\,m（门槛 $0.01$\,m），保持 240 步无异常。环境碰撞、自碰撞和非手指 cube 接触均为零，科学门控通过。该对照直接验证了以下因果链：斜视相机→表面点 XY$\neq$质心 XY→夹爪中心偏移→边缘抓取→抬升滑脱；头顶相机→表面点 XY$\approx$质心 XY→夹爪中心对准→扎实包裹→抬升成功。根因确在相机几何，不在控制链、后端选择或仿真参数。
...
补充 PyBullet 结果按设计顺序呈现了四层证据：(1) 九点坐标反投影门控关闭了从二维中心到世界点的数据流缺口；(2) 真值姿态五阶段控制链全部通过，证明运动-接近-闭合-抬升链路在仿真环境中可行；(3) 两个感知后端在斜视相机下均通过全部静态与到达门控，但抬升阶段因 XY 中心偏差（$24.67$--$26.62$\,mm，超过 cube 半宽）一致失败，确认根因不在后端选择；(4) 头顶相机将同一管线的 XY 偏差降至 $0.76$\,mm，从 pregrasp 到抬升保持全部通过，实现感知驱动的完整物理抓取成功。这四层证据按依赖顺序构建：先验证软件数据流正确，再证明控制链可执行，接着诊断感知失败根因，最后通过控制变量实验验证修复有效。它们不是独立抓取试验，不能据此计算感知成功率或后端排名。
...
这一分层设计对于解释失败根因至关重要。如果只做坐标验证而跳过真值控制链，无法区分"控制链本身不可行"与"感知输入错误"；如果只报告感知静态预检而不执行物理抓取，则 $26.55$\,mm 的中心偏差仍是推测而非观测。两个后端的配对失败排除了后端选择作为根因——geometry（$26.62$\,mm）和 multi-head（$24.67$\,mm）在同一阶段以相同模式失败，指向的是共享前端（斜视相机下的单像素反投影）。头顶相机的成功则完成了因果闭环：在仅改变相机位姿的条件下，同一感知管线和执行链条从失败翻转为成功（XY 偏差 $26.55\rightarrow0.76$\,mm，抬升 $0.0004\rightarrow0.1199$\,m），确认根因确在相机几何而非控制链、后端或仿真参数。该结论受限于固定场景 $N=1$ 和当前夹爪/物体几何；夹爪张开更大或物体更宽时边缘抓取可能偶然成功，但这不改变根因诊断。头顶方案证明了调整相机位姿即可在不增加传感器成本的条件下消除偏差，为实际部署提供了可操作的设计原则。
...
\section{贡献与实践意义}
研究贡献包括：建立定位前端与抓取后端可替换的三阶段比较；完成单头/多头的确定性五 seed 对照；生成并审计共同 image-wise 五折 manifest；以逐样本配对揭示空间回归与方向先验的互补；用 JSON、CSV、哈希和结果溯源约束论文主张。项目还为一个固定 PyBullet 场景提供了从坐标反投影到物理抓取的完整分层证据链：九点门控验证了感知数据流，真值控制链验证了仿真执行可行性，两个后端的斜视感知抓取确认了三维中心偏差是物理失败根因，头顶相机对照实验则将偏差从 $26.55$\,mm 降至 $0.76$\,mm 并实现了完整的感知驱动抓取成功。这四层证据从软件数据流到物理执行再到根因诊断与修复验证，构成了可追溯的闭环管线；其核心发现——调整相机位姿可消除单目反投影偏差——为实际部署提供了无需增加传感器成本的工程指导。实践上，该结构也便于在有限算力中先验证数据流和运动控制，再诊断感知瓶颈，避免将静态门控通过误判为物理成功。
...
\section{局限与未来工作}
结论受 Cornell 小数据、RGB-only、单矩形、通用 prompt、image-wise 视角泄漏和离线指标限制；矩形成功不等同于物理抓取。仿真物理抓取限于固定场景 $N=1$、单物体和固定夹爪/物体几何；头顶相机对照虽验证了相机位姿对偏差的控制作用，但仍需在随机物体位姿、多视角和多物体条件下检验结论的广泛性。下一步应恢复权威图像到物体映射并实施 object-wise 五折，加入 RGB-D 与密集多抓取输出，正式测试”学习中心/尺度 + 几何方向”的混合模型，并分析 prompt 与框阈值敏感性。若继续仿真执行，应在当前头顶方案基础上增加随机物体位姿和多视角相机，扩大抓取试验样本量，并探究利用局部深度结构（如窗口内中位数或法向估计）改善三维中心定位的可行性。迁移到真实机器人时还必须加入相机到机器人标定和在线感知-执行闭环。
