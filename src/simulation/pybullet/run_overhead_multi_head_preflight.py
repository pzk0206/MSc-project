"""Stage 6A overhead-camera multi-head preflight: single-pixel backprojection.

Runs the Stage 6A perception pipeline for the multi-head CNN backend using
an overhead (top-down) camera instead of the default oblique camera.

Differences from the Stage 6A overhead geometry preflight
(``run_overhead_preflight``):
  * backend is multi-head CNN: loads the frozen seed-42 multi-head weights
    and predicts the 2-D grasp with the CNN model
...
The resulting ``PerceptionExecutionPlan`` uses the multi-head overhead
protocol with the overhead deep-grasp control so that Stage 6B can execute
the physical grasp directly.
"""
...
DEFAULT_OUTPUT_DIR = Path(
    ...
    "stage_overhead_multi_head_preflight"
)
...
class MultiHeadOverheadPreflightConfig:
    """Frozen Stage 6A overhead multi-head preflight protocol."""
    ...
    def __post_init__(self) -> None:
        if self.seed != 42:
            raise ValueError(
                "Stage 6A overhead multi-head seed must be 42"
            )
        if self.target_name != "cube":
            raise ValueError(
                "Stage 6A overhead multi-head target_name must be cube"
            )
        if self.prompt != "red cube":
            raise ValueError(
                "Stage 6A overhead multi-head prompt must be red cube"
            )
        if self.backend != BACKEND:
            raise ValueError(
                "Stage 6A overhead multi-head backend must be multi_head"
            )
        if self.model_id != MODEL_ID:
            raise ValueError(
                "Stage 6A overhead multi-head model_id is frozen"
            )
        if (self.width, self.height) != (640, 480):
            raise ValueError(
                "Stage 6A overhead multi-head camera must be 640x480"
            )
        for name in ("box_threshold", "text_threshold", "iou_threshold"):
            if not math.isclose(getattr(self, name), 0.25, abs_tol=0.0):
                raise ValueError(
                    f"Stage 6A overhead multi-head {name} must be 0.25"
                )
...
def _base_metadata(config: MultiHeadOverheadPreflightConfig) -> dict[str, Any]:
    return {
        "timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "protocol": PROTOCOL_VERSION_OVERHEAD_MULTI_HEAD,
...
def run_overhead_multi_head_preflight(
    config: MultiHeadOverheadPreflightConfig,
    # trunk-ignore(pyright/reportMissingParameterType)
    dependencies=None,
) -> dict[str, object]:
    """Run the overhead multi-head preflight with single-pixel backprojection."""
    ...
    with PyBulletScene(scene_config) as scene:
        ...
        _save_image(
            output_dir / "multi_head_prediction.png",
            draw_prediction(
                frame.rgb,
                localization.box,
                grasp,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Stage 6A overhead-camera multi-head preflight: single-pixel "
    ...
    summary = run_overhead_multi_head_preflight(
        MultiHeadOverheadPreflightConfig(
