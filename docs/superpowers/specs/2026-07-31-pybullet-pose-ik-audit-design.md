...
自碰撞过滤只忽略同一 link、直接父子 link，以及由固定关节连接成同一刚体的
link 对；该固定关节闭包从 URDF joint metadata 推导，不按碰撞结果临时添加
白名单。固定研究场景还明确豁免 Panda base link 与 table 的安装接触；它是
固定底座边界而非候选轨迹碰撞。目标物体不豁免，因为当前姿态是表面上方的
standoff，而不是允许接触的闭合姿态。
