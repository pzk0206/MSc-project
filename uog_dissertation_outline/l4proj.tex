...
\section{与既有文献的关系}
本项目沿用 Jiang 的二维矩形和 Cornell 判定框架\citep{jiang2011efficient}。Lenz 的 RGB-D 两阶段候选评分证明了深度特征与候选重排的价值，并同时报告 image-wise/object-wise\citep{lenz2015deep}；Redmon 则把搜索改为全局直接回归和网格多抓取\citep{redmon2015real}。本研究的轻量 CNN 与 Redmon 同属“图像到矩形”范式，但输入是 Grounding DINO 产生的 RGB crop，fold 成员也由本项目生成，因此数值只能有限可比。
...
Figure~\ref{fig:backend-failures} 中，CNN-only 的 10/pcd1008、10/pcd1013、10/pcd1005 和 10/pcd1012，几何 IoU 为 0.145--0.206，CNN 为 0.489--0.627；可确认的是学习矩形更接近标注范围，短边规则低估尺度只是待验证解释。Geometry-only 中，10/pcd1003 的 CNN IoU 仅 0.127，而 10/pcd1001、09/pcd0945、09/pcd0947 的 CNN 角度超过 $30^\circ$，所以不能给四例分配同一原因。
...
\section{研究问题回答}
\textbf{研究问题一：}在当前 Cornell RGB、通用 \texttt{small object} 提示和既定阈值下，Grounding DINO 定位框明显改善了原整图几何流程。成功率由 56.95\% 升至 73.33\%，IoU 与角度指标也同步改善。这证明定位范围对当前基线重要，但不证明任意 VLM 在任意场景都优于任务专用检测器。
...
\section{补充图}
按目录平衡抽样的数据接触图见 Figure~\ref{fig:cornell-splits}，按结果类别选择的后端配对案例见 Figure~\ref{fig:backend-failures}。用于重新生成两幅图的源记录分别为 \path{representative_samples.csv} 和 \path{sample_comparison.csv}。
