...
| Global Average Pooling | `256×7×7 → 256` | 全局池化是轻量分类/回归网络的常见压缩方式；现代密集抓取网络则通常保留空间特征 | 相比展平层显著减少参数，并使回归头更小 | **是，自主工程选择** |
...
| 训练停止 | 最大 60 epoch，验证损失 early stopping，patience 10 | 标准模型选择流程 | 避免在 600 张训练图像上继续拟合；只用验证集选择 checkpoint | 否；阈值为自主工程选择 |
...
| 汇聚 | Adaptive Global Average Pooling | `256` |
| 回归头 | FC 128 + FC 64 | `64` |
