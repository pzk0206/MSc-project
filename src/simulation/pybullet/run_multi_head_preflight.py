"""Stage 6A multi-head preflight: single-pixel backprojection, no centre recovery.

Runs the Stage 6A perception pipeline for the multi-head CNN backend only.
Unlike the Stage 6A.2 protocol it does NOT apply windowed top-surface centre
recovery: the 2-D CNN grasp centre is backprojected with the original
single-pixel nearest-depth method (``sample_nearest_depth`` +
``backproject_pixel``), matching Stage 6A geometry.  The resulting
``PerceptionExecutionPlan`` uses the V2 protocol with ``center_recovery=None``
so that Stage 6B can execute the physical grasp directly.
"""
...
class MultiHeadPreflightConfig:
    """Frozen Stage 6A multi-head preflight protocol."""
    ...
    def __post_init__(self) -> None:
        if self.seed != 42:
            raise ValueError("Stage 6A multi-head seed must be 42")
        if self.target_name != "cube":
            raise ValueError("Stage 6A multi-head target_name must be cube")
        if self.prompt != "red cube":
            raise ValueError("Stage 6A multi-head prompt must be red cube")
        if self.model_id != MODEL_ID:
            raise ValueError("Stage 6A multi-head model_id is frozen")
        if (self.width, self.height) != (640, 480):
            raise ValueError("Stage 6A multi-head camera must be 640x480")
        for name in ("box_threshold", "text_threshold", "iou_threshold"):
            if not math.isclose(getattr(self, name), 0.25, abs_tol=0.0):
                raise ValueError(
                    f"Stage 6A multi-head {name} must be 0.25"
                )
...
def _base_metadata(config: MultiHeadPreflightConfig) -> dict[str, Any]:
    return {
...
def run_multi_head_preflight(
    config: MultiHeadPreflightConfig,
    # trunk-ignore(pyright/reportMissingParameterType)
    dependencies=None,
) -> dict[str, object]:
    """Run the multi-head CNN preflight with single-pixel backprojection."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Stage 6A multi-head preflight: single-pixel backprojection "
            "without centre recovery."
        )
    )
    ...
    summary = run_multi_head_preflight(
        MultiHeadPreflightConfig(
