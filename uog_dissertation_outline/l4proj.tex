...
\section{研究范围与意义}
三个主要研究问题仍限定于 Cornell 数据集上的离线二维 RGB 感知，不在这些性能比较中融合深度。论文另以固定 PyBullet 场景进行补充系统验证，检验保存的二维中心能否通过米制深度和相机矩阵恢复为目标表面世界坐标；该验证不进入 Cornell 排名，也不涉及完整抓取姿态、碰撞检测、轨迹规划、力控制或真实抓取执行。由于缺少权威图像到物体实例映射，本文不把当前 image-wise 五折解释为 object-wise 未见物体泛化；矩形成功也不等同于物理机器人成功。通用提示被固定为控制变量，prompt 搜索与 VLM 微调属于后续工作。

学术上，本研究通过共享定位裁剪减少模块混杂，补充 VLM 前端与轻量抓取后端之间的受控证据。工程上，定位器和后端可以独立替换，便于在算力有限的平台上诊断误差来源。更重要的是，论文同时报告均值、波动和逐样本互补性，使结论强度与现有证据相匹配。

\section{论文结构}
第二章按技术主题批判性回顾抓取矩形、深度学习、开放词汇定位和语言驱动抓取，并提出研究缺口。第三章说明数据、三条流程、训练协议、评价方法、研究伦理及独立的 PyBullet 坐标验证方法。第四章按三个研究问题报告 Cornell 定量和定性结果，并另列补充坐标验证；第五章联系既有文献解释发现及限制，第六章逐条回答研究问题并提出未来工作。附录记录复现命令、环境、结果产物与固定划分的历史用途。
...
\section{PyBullet 补充坐标验证方法}
补充验证采用一个固定 PyBullet 场景，其中包含黄色鸭、红色方块、绿色球体和作为干扰项的 Franka Panda。三条明确目标提示和 geometry、single、multi-head 三个后端复用同一幅 $640\times480$ RGB 图像、米制深度、segmentation、view matrix 与 projection matrix。只有正确选择目标后保存的九个二维抓取中心进入坐标验证；该过程发生在二维预测之后，不改变 Grounding DINO 或三个后端的输入。

对每个中心 $(c_x,c_y)$，系统采用最近像素并以 half-up 规则取整得到列与行，然后读取该像素的米制深度。米制深度先按 PyBullet/OpenGL 透视深度关系还原为 depth-buffer 值，再使用列主序 projection matrix 的逆矩阵恢复相机齐次坐标，并使用 projection--view 组合矩阵的逆矩阵恢复世界坐标。所得世界点随后通过原 view/projection matrix 重投影到图像。相机矩阵和深度缓冲约定遵循 Bullet Physics 的 PyBullet Quickstart Guide\footnote{\url{https://github.com/bulletphysics/bullet3/blob/master/docs/pybullet_quickstartguide.pdf}}；最近像素选择、九点协议和联合门槛为本项目的工程设计。

逐点门控要求深度严格位于 $0.05$\,m 与 $3.0$\,m 裁剪面之间、全部相机与世界坐标有限、像素重投影误差不超过 $1$\,px、深度往返误差不超过 $10^{-4}$\,m、采样像素的 segmentation body 与指定目标一致，且从相机光心穿过反投影点并延长 $0.05$\,m 的 \texttt{rayTest} 首次命中同一目标。Segmentation 与射线结果只在坐标产生后作真值审计，不修正坐标，也不参与二维预测。该阶段不生成完整夹爪姿态，不调用 IK，不移动关节，也不闭合夹爪。
...
\section{方法局限}
固定目录接触图不能证明难度相等；image-wise 五折仍可能有相邻视角信息泄漏。由于现有文件缺少可审计的 885 图像到物体实例映射，本文不报告 object-wise。Grounding DINO 可能对 prompt 和阈值敏感，但本研究未做提示搜索。CNN 只用小规模 RGB crop 并输出单矩形，而现代方法常利用 RGB-D 与密集多抓取图。Cornell 的最终评价仍是 RGB-only offline metric；PyBullet 补充验证只恢复中心对应的目标表面点，不估计完整抓取姿态、运动学可达性、碰撞或接触稳定性，因此二者都不等同于物理抓取。
...
九点中的最大像素重投影误差为 $8.04\times10^{-13}$\,px，最大深度往返误差为 $3.57\times10^{-7}$\,m，均远低于预先固定的工程门槛。反投影点到射线实际命中位置的最大距离为 $0.00676$\,m；该值受渲染深度表面与碰撞几何差异影响，只作为诊断量，不增加新的通过条件。由于九条记录来自同一确定性场景且共享上游图像，它们不是九次独立抓取试验，不能据此计算具有总体意义的成功率、置信区间或后端排序。
...
补充 PyBullet 结果进一步表明，在这一固定场景中，保存的二维中心能够一致地转换为经事后目标审计的表面世界点。这一结果不回答第四个研究问题，也不验证完整姿态、IK 可达性、无碰撞运动或物理抓取；它只关闭了从二维中心、米制深度和相机矩阵到三维表面点的数据流门控。
...
\section{从二维中心到世界坐标的补充证据}
PyBullet 九点门控关闭了一个较窄但必要的接口缺口：二维中心、米制深度和相机矩阵能够在当前实现中产生数值有限、可重投影且命中指定目标的世界点。因此，9/9 首先验证的是软件数据流、矩阵顺序、图像坐标方向和深度换算约定，而不是抓取动作本身。九条记录还是同一图像和同一确定性场景产生的相关输出，不能当作独立试验估计总体不确定性，也不能用于判定 geometry、single 或 multi-head 哪个后端更适合物理执行。

后续证据需要保持分层：坐标一致不等于抓取姿态有效，因为单个表面点尚未确定接近轴和夹爪绕轴方向；IK 可达不等于运动无碰撞，因为关节解和预抓取路径仍可能穿过桌面、机器人或目标；无碰撞到达也不等于稳定抓取和抬升，因为还缺少接触、闭合、摩擦与保持判据。当前补充结果只支持进入下一层设计，不能跨越这些门控。

\section{研究局限}
第一，研究只用 Cornell 小型单物体数据，不能代表杂乱开放世界。第二，Cornell 训练与评价输入保持 RGB-only；补充 PyBullet 验证只在二维预测完成后使用深度恢复一个表面点，未利用局部深度结构估计法向或完整姿态。第三，image-wise 允许相邻视角跨集合，权威物体映射缺失使 object-wise 暂不可审计。第四，固定 85 样本较小且难度偏置明显。第五，通用 prompt、框阈值和 10\% 扩张未做敏感性实验，885/885 定位覆盖率也不是定位质量。
...
\section{贡献与实践意义}
研究贡献包括：建立定位前端与抓取后端可替换的三阶段比较；完成单头/多头的确定性五 seed 对照；生成并审计共同 image-wise 五折 manifest；以逐样本配对揭示空间回归与方向先验的互补；用 JSON、CSV、哈希和结果溯源约束论文主张。项目还为一个固定 PyBullet 场景提供了可复现的二维中心到世界表面点门控；它是用于连接感知输出的集成产物，不是新的抓取规划算法。实践上，该结构便于在有限算力中先诊断目标选择，再替换局部后端，而不必把大型 VLM 重新训练成完整抓取器。
...
\section{局限与未来工作}
结论受 Cornell 小数据、RGB-only、单矩形、通用 prompt、image-wise 视角泄漏和离线指标限制；矩形成功不等同于物理抓取。下一步应恢复权威图像到物体映射并实施 object-wise 五折，加入 RGB-D 与密集多抓取输出，正式测试“学习中心/尺度 + 几何方向”的混合模型，并分析 prompt 与框阈值敏感性。若继续仿真执行，应按依赖顺序设计抓取方向与预抓取姿态、检验 IK 可达性、检查静态与路径碰撞、生成并执行轨迹、闭合夹爪，再以接触和抬升判据评价结果；迁移到真实机器人时还必须加入相机到机器人标定。上述执行环节均未在本研究中实现。
...
PyBullet metadata 保留以下证据边界：\texttt{depth\_used\_after\_2d\_prediction: true}，\texttt{segmentation\_used\_as\_coordinate\_input: false}，\texttt{ray\_test\_used\_as\_coordinate\_input: false}，\texttt{ik\_executed: false}，以及 \texttt{physical\_grasp\_executed: false}。这些字段用于防止把事后真值审计写成坐标输入，或把坐标门控写成机械臂执行。
