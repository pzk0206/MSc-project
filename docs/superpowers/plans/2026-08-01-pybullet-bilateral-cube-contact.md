...
**目标：**从已通过的 cube 接触前姿态缓慢闭合 Panda 双指，首次获得双指同时目标接触后保持，并用真实 body/link/法向力证明接触成立；本阶段不抬升。
...
surface-standoff 并求 IK；测试准备阶段只用 `resetJointState` 将手臂放到该
...
要求轨迹包含 `pregrasp/approach/close/contact_hold`，接触事件非空且 link 集合
等于两个手指；四张 PNG 为 640×480。metadata 中闭合、接触评价和目标接触为
真，抬升和物理抓取为假。
...
将该阶段纳入与 OPEN_APPROACH 相同的双姿态预检和两段电机执行；只有 approach
全部动态门控通过，才在同一 scene/client 中调用：
...
四张 640×480 PNG。人工查看 closed 图，确认双指位于 cube 两侧且 cube 仍在
桌面上。
